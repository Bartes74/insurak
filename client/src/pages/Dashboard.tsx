    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
          ...
          <div>
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="mt-1 text-sm text-gray-500">Brak pilnych zadań do wykonania.</p>
              </div>
            ) : (
                  ...
                  <li key={item.assetId}>
                    <Link
                      ...
                      className={`group flex items-start space-x-4 rounded-2xl border p-4 transition-all hover:shadow-md ${item.severity === 'danger'
                        ...
                        : item.severity === 'warning'
                          ? 'border-orange-100 bg-orange-50/50 hover:bg-orange-50 hover:border-orange-200'
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className={`text-sm font-medium ${item.severity === 'danger' ? 'text-red-900 line-through decoration-red-300'
                          : item.severity === 'warning' ? 'text-orange-900'
                            : 'text-blue-900'
                          }`}>
                          {item.assetName}
                        </p>
                        <p className={`text-sm mt-0.5 ${item.severity === 'danger' ? 'text-red-700'
                          : item.severity === 'warning' ? 'text-orange-700'
                            : 'text-blue-700'
                          }`}>
                          {item.message}
                        </p>
        ...
        <Card className="h-full">
          ...
          {cashflow.length === 0 ? (
            <div className="mt-5 h-48 flex items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl">
